...
public class Erosion implements Filter {
    ...
    public void apply(ProcessImage image) {
        ...
        IntStream.range(1, width - 1).parallel().forEach((x) ->
            IntStream.range(1, height - 1).parallel().forEach((y) -> {
                ...
                if(scan == 4) {
                    ...
                    boolean black = pixelList.stream().anyMatch((pixel) -> (pixel & 0xFF) == 0);
        ...
        */
        ...
        serie.setName("Erosion");

        boolean exists = image.getSeries().stream().anyMatch((item) -> item.getName() == "Erosion");
